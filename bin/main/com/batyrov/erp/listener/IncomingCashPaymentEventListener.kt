...
@Component("erp_IncomingCashPaymentEventListener")
open class IncomingCashPaymentEventListener {
    @Autowired
    private val dataManager: DataManager? = null
    private var logger: Logger = Logger.getLogger("IncomingPaymentEventListener");

    @EventListener
    open fun onIncomingCashPaymentChangedBeforeCommit(event: EntityChangedEvent<IncomingCashPayment>) {
        var cal = Calendar.getInstance()
        when (event.type) {
            EntityChangedEvent.Type.CREATED -> {
                var incomeLedger = dataManager!!.create(IncomeLedger::class.java)
                val incomingPayment: IncomingCashPayment = dataManager.load(event.entityId).one();
