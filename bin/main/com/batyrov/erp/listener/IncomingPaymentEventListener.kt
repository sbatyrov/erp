...
@Component("erp_IncomingPaymentEventListener")
open class IncomingPaymentEventListener {
    @Autowired
    private val dataManager: DataManager? = null
    var logger: Logger = Logger.getLogger("IncomingPaymentEventListener");

    @EventListener
    open fun onIncomingPaymentChangedBeforeCommit(event: EntityChangedEvent<IncomingPayment>) {
        var cal = Calendar.getInstance()
        when (event.type) {
            EntityChangedEvent.Type.CREATED -> {
                var incomeLedger = dataManager!!.create(IncomeLedger::class.java)
                val incomingPayment: IncomingPayment = dataManager.load(event.entityId).one();
